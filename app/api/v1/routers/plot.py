...
async def get_names_by_type(
    ...
    return await plot_service.get_names_by_type(current_user.id, journal_id, account_type)
...
async def get_counts_by_acc(
    ...
    return await plot_service.get_account_entry(current_user.id, journal_id, account_type)
...
async def get_htmx_activity(
    ...
    counts = await plot_service.get_account_entry(current_user.id, journal_id, account_type)
...
@router.get("/htmx-market-price", response_class=HTMLResponse, summary="Market Price Chart")
async def get_market_price_chart(
    ...
    data = [
        {"Pair": f"{p.currency_from}/{p.currency_to}", "Date": p.date, "Price": float(p.price)}
        for p in prices
    ]
    df = pd.DataFrame(data).sort_values(by="Date")

    fig = px.line(df, x="Date", y="Price", color="Pair", markers=True)
    fig.update_layout(
